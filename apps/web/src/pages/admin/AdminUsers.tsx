...
const AdminUsers: Component = () => {
  ...
  // Risk Control State
  const [riskUserId, setRiskUserId] = createSignal<number | null>(null);
  const [riskValue, setRiskValue] = createSignal<number>(0);
  const [isSuperAdmin, setIsSuperAdmin] = createSignal(false);

  onMount(() => {
    fetchList();
    const role = localStorage.getItem('admin_role_type');
    setIsSuperAdmin(role === '0');
  });
  ...
  const handleSetRisk = async () => {
    const uid = riskUserId();
    if (!uid) return;
    setMsg('');
    try {
      // Use superadminApi for risk settings
      const res = await superadminApi.riskProfile.set(uid, riskValue());
      if (res.type === 'ok') {
        setMsg('风控设置已更新');
        setRiskUserId(null);
      } else {
        setMsg((res as any).message || '设置失败');
      }
    } catch (e) {
      setMsg('请求失败');
    }
  ...
  return (
    <div class="space-y-6">
                      ...
                      <td class="p-4 flex items-center gap-2">
                        ...
                        {isSuperAdmin() && (
                          <button
                            ...
                            onClick={() => { setRiskUserId(u.id); setRiskValue(0); }}
                          >
                            风控
                          </button>
                        )}
      ...
      {/* Risk Control Modal */}
      {riskUserId() !== null && (
        <div class="fixed inset-0 bg-black/50 flex items-center justify-center z-50" onClick={() => setRiskUserId(null)}>
          <div class="bg-white border border-gray-200 rounded-xl p-6 max-w-md w-full shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h3 class="text-lg font-semibold text-gray-900 mb-4">风控设置 (秒合约) - 用户 #{riskUserId()}</h3>
            <div class="space-y-4 mb-6">
              <label class="flex items-center space-x-3 p-3 rounded border border-gray-200 cursor-pointer hover:bg-gray-50">
                <input type="radio" name="risk" value="0" checked={riskValue() === 0} onChange={() => setRiskValue(0)} class="text-primary" />
                <div>
                  <div class="font-medium text-gray-900">正常 (随机/概率)</div>
                  <div class="text-xs text-gray-500">依据系统整体赔率运行</div>
                </div>
              </label>
              <label class="flex items-center space-x-3 p-3 rounded border border-green-200 bg-green-50 cursor-pointer hover:bg-green-100">
                <input type="radio" name="risk" value="1" checked={riskValue() === 1} onChange={() => setRiskValue(1)} class="text-green-600" />
                <div>
                  <div class="font-medium text-green-900">必赢 (指定盈利)</div>
                  <div class="text-xs text-green-700">用户下注必定盈利</div>
                </div>
              </label>
              <label class="flex items-center space-x-3 p-3 rounded border border-red-200 bg-red-50 cursor-pointer hover:bg-red-100">
                <input type="radio" name="risk" value="-1" checked={riskValue() === -1} onChange={() => setRiskValue(-1)} class="text-red-600" />
                <div>
                  <div class="font-medium text-red-900">必输 (指定亏损)</div>
                  <div class="text-xs text-red-700">用户下注必定亏损</div>
                </div>
              </label>
            </div>
            <div class="flex gap-2">
              <button type="button" class="px-4 py-2 rounded bg-primary text-white" onClick={handleSetRisk}>确认修改</button>
              <button type="button" class="px-4 py-2 rounded bg-gray-600 text-gray-300" onClick={() => setRiskUserId(null)}>取消</button>
            </div>
          </div>
        </div>
      )}
